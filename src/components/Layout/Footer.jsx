    ...
    return (
        <footer className="bg-theme pt-6 pb-5">
            <div className="nike-container text-slate-200">
                <div className="grid items-start grid-cols-3 sm:grid-cols-2 max-w-6xl w-full mx-auto gap-6 md:gap-4">
                    {titles.map((title, i) => (
                        <div key={i} className="grid items-center">
                            <h2 className="text-lg md:text-sm lg:text-base text-center sm:text-start uppercase font-semibold mb-2">
                                {title.title}
                            </h2>
                            <ul className="grid items-center gap-2 justify-items-center sm:justify-items-start">
                                {links[i]?.map((link, j) => (
