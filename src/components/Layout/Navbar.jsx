...
const Navbar = () => {
    ...
    const onNavScroll = () => {
        setNavState(window.scrollY > 10 ? true : false);
    }
                ...
                <ul className = "flex items-center justify-center gap-2">
                    ...
                    <li className = "grid items-center">
                        <button type = "button" className = "border-none outline-none active:scale-110 transition-all duration-300 relative" onClick = {onCartToggle}>
                            ...
                            <div className = {`flex items-center justify-center cursor-pointer hover:scale-110 transition-all duration-300 absolute top-4 right-0 shadow w-4 h-4 text-[0.65rem] leading-tight font-medium rounded-full ${navState ? "bg-slate-900 text-slate-100 shadow-slate-900" : "bg-slate-100 text-slate-900 shadow-slate-100"}`}>
                                {quantity}
                            </div>
                        </button>
                    </li>
